...
#' 
#' This function writes and saves the Mplus syntax given the specifications of a
...
syntax.mplus <- function(blocks,itf,model,input_path,data_path='myDataFile.dat',fscore_path='myFactorScores.dat',title='myFC_model',
  ...
  # Begin syntax
  # Init parameters for syntax (combine with init parameters above)
  title <- title
  estimator <- estimator
  ...
  # DATA
  input <- paste0(input, 'DATA: FILE IS ',data_path, ';\n\n')
  
  # VARIABLE
  input <- paste0(input, 'VARIABLE: NAMES ARE ', '\n')
  if(ID){
    input <- paste0(input, 'ID', '\n')
  }

  if(data_full){
    tmp <- which(!pair_names_b%in%pair_names)
    if(length(tmp)!=0){
      tmp1 <- pair_names_b[tmp]
      tmp2 <- sub('^i.+i','i',tmp1)
      for(i in 1:length(tmp)){
        tmp3 <- paste0(tmp2[i],sub(paste0(tmp2[i],'$'),'',tmp1[i]))
        pair_names[which(pair_names%in%tmp3)] <- pair_names_b[tmp[i]]
      }
    }
    
    input <- paste0(input, paste(pair_names,collapse = '\n'), ';\n')
  }else if(byblock){
    tmp <- apply(blocks,1, function(y) apply(t(utils::combn(y,2)),1,function(x) paste0('i',x[1],'i',x[2])))
    pair_names_bb <- paste0(tmp)
    input <- paste0(input, paste(pair_names_bb,collapse = '\n'), ';\n')
  }else{
    input <- paste0(input, paste(pair_names_bs,collapse = '\n'), ';\n')
  }
  
  if(design=='full'|!data_full){
    input <- paste0(input, '\n','USEVARIABLES ARE ALL',';\n')
  }else{
    input <- paste0(input, '\n','USEVARIABLES ARE ','\n')
    input <- paste0(input, paste(pair_names_b,collapse = '\n'), ';\n')
  }
  
  input <- paste0(input, 'CATEGORICAL ARE ALL;\n\n')
  
  if(ID){
    input <- paste0(input, 'IDVARIABLE IS ID;', '\n')
  }
  ...
  if(model%in%c('lmean','uc','simple')){
    ...
    input <- paste0(input, '! Model of primary factors:\n')
    t <- paste0('t',1:nitem)
    ...
    if(model!='simple'){
      input <- paste0(input, '! Model of secondary factors:\n')
      if(nfactor==1&Arank==(nitem-1)){
        ...
        for(i in 1:nfactor){
          ...
          #for(j in 1:length(gblocks)){
          #if(unidim_gblocks[j]){
          #if(length(items_fac_uni[[i]])>0&any(gblocks[[j]]%in%items_fac_uni[[i]])){
      ...
      input <- paste0(input, paste0(traits[1],'-',traits[nfactor], '@1', collapse = ' '),';', '\n\n')
      
      # Correlations between factors
      input <- paste0(input, '! Define the correlations between factors:\n')
      if(nfactor>1){
        inp_tmp <- paste0('Trait',t(utils::combn(1:nfactor,2))[,1], ' WITH ', 'Trait', t(utils::combn(1:nfactor,2))[,2],';', collapse = '\n')
      ...
      if(nrank==2){
        ...
        input <- paste0(input, paste0('t',1:nitem,collapse = '@1;\n'), '@1;\n')
      }else{
        input <- paste0(input, '!Fix uniquenesses of one item per meta block:\n')
        for(i in 1:length(gblocks)){
          tmp <- min(gblocks[[i]])
          input <- paste0(input,'t',tmp,'@1;\n')
        }
      }
    }
    ...
    if(model=='lmean'|model=='simple'){
      ...
      input <- paste0(input, '\n[',pair_names_b[1],'$1-',utils::tail(pair_names_b, n=1),'$1@0]',';\n\n')
      if(design=='full'){
        input <- paste0(input, '! Fix one mean to 0 and free all other means:\n')
        input <- paste0(input, '[t1@0]',';\n')
        input <- paste0(input, '[t2-t',nitem,']',';\n\n')
      }
      if(design=='block'){
        input <- paste0(input, '! Fix one mean per grand block to 0 and free all other means:\n')
        input <- paste0(input, '[t1-t',nitem,']',';\n')
        # This is not necessary but maybe more helpful for application. Fix the lowest item per block
        #####
        for(i in 1:length(gblocks)){
          tmp <- min(gblocks[[i]])
    ...
    if(model=='simple'){
      if(case==5){
        ...
        for(i in 1:(nitem-2)){
          input <- paste0(input,'\n',paste0('t',i),' WITH ',paste0('t',i+1),'-',paste0('t',nitem),'@0;')
        }
        input <- paste0(input,'\n',paste0('t',nitem-1),' WITH ',paste0('t',nitem),'@0;')
      }
      if(case==3){
      ...
      if(case==2){
        ...
        for(i in 1:(nitem-3)){
          input <- paste0(input,'\n',paste0('t',i),' WITH ',paste0('t',i+1),'-',paste0('t',nitem-1),'*;')
        }
        input <- paste0(input,'\n',paste0('t',nitem-2),' WITH ',paste0('t',nitem-1),'*;')
        input <- paste0(input,'\n',paste0('t',nitem),' WITH ',paste0('t1-t',nitem-1),'@0;')
      }
    }
